...
func PlaceOrder(
	...
	getProductPrice GetProductPrice, // 依存関係
        ...
        validateCustomerInfo ValidateCustomerInfo, // 依存関係（★新規追加）
	...
	return func(unvalidatedOrder api.UnvalidatedOrder) []api.OrderEvent {
		...
		pricedOrder := PriceOrder(validatedOrder, getProductPrice)
                ...
                pricedOrderWithVipShipping := FreeVipShipping(pricedOrderWithShippingInfo, calculateVipShippingCost) // VIP用配送料計算（★新規追加）
...
##### 変更前
```go
func calculateShippingCostWithCategory(address Address) float64 {
	switch categorizeAddress(address) {
	case UsLocalState:
		return 5.0
	case UsRemoteState:
		return 10.0
